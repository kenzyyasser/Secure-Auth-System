...
const logout = (req, res) => {
  try {
    ...
    db.prepare('DELETE FROM token_blacklist WHERE expires_at < CURRENT_TIMESTAMP').run();

    logEvent(req.user.userId, req.user.email, 'LOGOUT', req, 'Token blacklisted');
...
const verifyBackupCode = async (req, res) => {
  ...
  if (remainingCount <= 2) {
    ...
    logEvent(userId, user.email, 'BACKUP_CODES_AUTO_GENERATED', req, 'Generated 8 new codes');
  }
